namespace PushSharp.Google
{
    ...
    public class GcmServiceConnection : IServiceConnection<GcmNotification>
    {
        ...
        private async Task processResponseOk(HttpResponseMessage httpResponse, GcmNotification notification)
        {
            ...
            foreach (var r in result.Results)
            {
                ...
                else if (r.ResponseStatus == GcmResponseStatus.NotRegistered)
                {
                    ...
                    multicastResult.Failed.Add(
                        singleResultNotification,
                        new DeviceSubscriptionExpiredException(singleResultNotification)
                        {
                            OldSubscriptionId = oldRegistrationId
                        });
        ...
        private async Task processResponseError(HttpResponseMessage httpResponse, GcmNotification notification)
        {
